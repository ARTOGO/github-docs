...
| `notion-pr-sync-reusable.yml` | reusable | 各 repo PR 事件觸發 | SB- branch PR sync（開啟→TECH REVIEW、review→DEV IN PROGRESS、merge→STAGING/PROD FUNC REVIEW）；Feature Hub 子卡查找（by repo + branch match）；Feature Hub rollup（所有子卡到位→更新 Hub 狀態 + 通知 Func. Reviewer）；FIX- branch PR sync（Fix Task 狀態 + Fix PR 欄位）；Fix PR merge → 檢查所有 Fix Task 完成 → Review Issue → Tech Fixed |
...
- PR sync workflow 擴充：`FIX-` prefix branch 的 PR 事件同步 Fix Task 狀態
  - Fix PR opened → 通知 Tech Reviewer（從 Repo Execution 卡或 Feature Hub 繼承）
  - Fix PR review → 通知修復者
...
| 事件 | 條件 | 狀態更新 | 通知 |
|---|---|---|---|
| PR opened/reopened | — | TECH REVIEW + PR Status=Open | Tech Reviewer |
