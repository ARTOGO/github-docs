...
| `notion-pr-sync-reusable.yml` | reusable | 各 repo PR 事件觸發 | SB- branch PR sync（開啟→PR Status=Open、狀態→TECH REVIEW、通知 Tech Reviewer；review→DEV IN PROGRESS；merge→STAGING/PROD FUNC REVIEW）；Feature Hub 子卡查找（by repo + branch match）；Feature Hub rollup（所有子卡到位→更新 Hub 狀態 + 通知 Func. Reviewer）；FIX- branch PR sync（Fix Task 狀態 + Fix PR 欄位；opened 時通知 Tech Reviewer）；Fix PR merge → 檢查所有 Fix Task 完成 → Review Issue → Tech Fixed |
...
- PR sync workflow 擴充：`FIX-` prefix branch 的 PR 事件同步 Fix Task 狀態
  - Fix PR opened → Fix PR Status = Open；通知 Tech Reviewer
  - Fix PR review → 通知修復者
...
branch 自動建立（從 default branch 切出） → 回寫 GitHub Branch
  ↓ Developer 開發完成 → 開 PR
PR opened → PR Status = Open + 狀態 TECH REVIEW + 通知 Tech Reviewer
...
| 事件 | 條件 | 狀態更新 | 通知 |
|---|---|---|---|
| PR opened/reopened | — | PR Status=Open + TECH REVIEW | Tech Reviewer |
