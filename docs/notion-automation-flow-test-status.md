...
最後更新：2026-05-22 18:56 Asia/Taipei
...
| Reviewer event / Notion mention 補測 | 部分通過，部分等待真人 reviewer | `SB-2222` / `SB-2223`、tool-imagecompressor PR #10 / #11、runs `26282142330`, `26282194741`, `26282238457`, `26282312397`, `26282354736`, `26282409153`, `26282501635` |
...
GitHub `review_requested` 已 live 測。`ISS-*` repair PR review `commented` 已由 Gemini Code Assist 的真實 `pull_request_review` 測到。`SB-*` review `commented`、`SB-*` / `ISS-*` review `changes_requested` 與 `approved` 仍等待非 PR author 的真人 reviewer；目前 PR #10 / #11 已 request 並 tag `zeallin`。
...
| PR review `changes_requested` -> `DEV IN PROGRESS` | 已實作 | blocked | 等待 `zeallin` 在 PR #10 送出真人 GitHub review；PR author 不能自測，workflow 也會跳過同作者 review。 |
| PR review `commented` -> `DEV IN PROGRESS` | 已實作 | blocked | 等待 `zeallin` 在 PR #10 送出真人 GitHub review。Gemini 在空 PR #10 留的是 issue comment，不是 `pull_request_review`。 |
| PR review `approved` 不改狀態，等待 merge | 已實作 | blocked | 等待 `zeallin` 在 PR #10 送出真人 GitHub review。 |
...
| repair PR review `changes_requested` -> comment 通知 developer / assignee / fixer | 已實作 | blocked | 等待 `zeallin` 在 PR #11 送出真人 GitHub review；PR author 不能自測，workflow 也會跳過同作者 review。 |
...
| repair PR `approved` 不改狀態，等待 merge | 已實作 | blocked | 等待 `zeallin` 在 PR #11 送出真人 GitHub review。 |
...
1. `SB-*` PR review `commented/changes_requested -> DEV IN PROGRESS`：等待 `zeallin` 在 PR #10 送出真人 GitHub review。
2. `SB-*` PR `approved` 不改狀態：等待 `zeallin` 在 PR #10 送出真人 GitHub review。
3. `ISS-*` repair PR review `changes_requested`：等待 `zeallin` 在 PR #11 送出真人 GitHub review。
4. `ISS-*` repair PR `approved` 不改狀態：等待 `zeallin` 在 PR #11 送出真人 GitHub review。

補充：PR #10 / #11 已 request 並 tag `zeallin`。Gemini 在空 commit PR #10 / #11 留的是 regular PR issue comment，不是 `pull_request_review`，不能用來驗證 `SB-*` review submitted path。
