...
| Notion Worker / Notion Agent | Worker tools 已實作並 live 測；Custom Agent UI 未完成 | `workers/notion-product-workflow` 已新增 route / reroute / complete tools，run `26337196957` 用真實 `ISS-203` 驗證；hosted Worker deploy 與 Custom Agent trigger 仍需 `ntn login` / Notion UI。 |
...
這一段目前已完成 Worker-compatible tool implementation 與 live side-effect test；hosted Notion Worker deploy 與 Custom Agent trigger/access 設定仍需要 Notion CLI workspace login 與 Notion UI。

| 流程 | 實作狀態 | 測試狀態 | 備註 |
| --- | --- | --- | --- |
| `Review Issue -> Open` 後自動 route affected repos | Worker tool 已實作 | 已 live 測 tool | `routeReviewIssue` / `rerouteReviewIssue` 由 run `26337196957` 建立 `ISS-203` 並 route 到 `SB-2223` / `tool-imagecompressor`。Notion trigger 尚需 Custom Agent UI 設定。 |
| route 後自動寫回 `Affected Repo Execution` | Worker tool 已實作 | 已 live 測 | `ISS-203` 寫入 `Affected Repo Execution = SB-2223`。 |
| route 後自動寫回 `Repair Routing Summary` | Worker tool 已實作 | 已 live 測 | `ISS-203` 寫入 `[Notion Worker] Review Issue routing` summary。 |
| route 後自動 tag 對應 repo developer / assignee | Worker tool 已實作 | 已 live 測 | `ISS-203` comment mention Zeal Lin：「Worker 已判斷此 Review Issue 需要修復 repo」。 |
| `Review Issue` 被改回 `Open` 後自動清空 resolved / PR 摘要並 reroute | Worker tool 已實作 | 已 live 測 tool | `rerouteReviewIssue` 在 `ISS-203` live test 中清空舊 `Repair PR URLs`，重寫 affected，並將 `Reopen Count` 改為 `1`；Notion update trigger 尚需 Custom Agent UI 設定。 |
| 所有 affected repo 完成後由 Worker `complete_review_issue_if_ready` 判斷 | Worker tool 已實作 | 已 live 測 | `completeReviewIssueIfReady` 在 `ISS-203` 上確認 resolved 覆蓋 affected，並將狀態改為 `Tech Fixed`。 |
| Feature Hub 底下所有 Review Issue 都不再 `Open/Fixing` 時通知 tester retest | Worker guard 已實作 | 未完整 live 測通知 | Worker 會計算 retest readiness；只有 Feature Hub relation 有可判斷 issue set 且全部非 active 時才留言，避免孤立測試卡誤通知。 |
