...
開始開發前，請確認以下 AI Skills 已安裝。若未安裝，先執行安裝指令：

```bash
# 檢查是否已安裝
ls ~/.claude/skills/gstack/SKILL.md 2>/dev/null && echo "gstack ✅" || echo "gstack ❌ 需安裝"
ls ~/.claude/skills/ui-ux-pro-max/.claude/skills/ui-ux-pro-max/SKILL.md 2>/dev/null && echo "ui-ux-pro-max ✅" || echo "ui-ux-pro-max ❌ 需安裝"
ls ~/.claude/skills/senior-frontend/SKILL.md 2>/dev/null && echo "senior-frontend ✅" || echo "senior-frontend ❌ 需安裝"
ls ~/.claude/skills/senior-backend/SKILL.md 2>/dev/null && echo "senior-backend ✅" || echo "senior-backend ❌ 需安裝"
ls ~/.claude/skills/tdd-guide/SKILL.md 2>/dev/null && echo "tdd-guide ✅" || echo "tdd-guide ❌ 需安裝"
```
...
# senior-frontend / senior-backend / tdd-guide — 開發實作 Skills
git clone --single-branch --depth 1 https://github.com/alirezarezvani/claude-skills.git /tmp/_cs \
  && cp -R /tmp/_cs/engineering-team/senior-frontend ~/.claude/skills/senior-frontend \
  && cp -R /tmp/_cs/engineering-team/senior-backend ~/.claude/skills/senior-backend \
  && cp -R /tmp/_cs/engineering-team/tdd-guide ~/.claude/skills/tdd-guide \
  && rm -rf /tmp/_cs
```
...
- 根據開發需求進行規劃，產出 PRD
- **可用 Skill**：gstack `/office-hours`（挑戰假設）、`/plan-ceo-review`（審視 scope）
...
- **可用 Skill**：
  - `ui-ux-pro-max` — 自動產生設計系統（色彩、字型、元件風格）
...
- **可用 Skill**：gstack `/plan-eng-review`（架構、資料流、edge cases、測試覆蓋審查）
...
- **TDD 原則**：先寫失敗測試，再寫實作，最後重構。使用 `tdd-guide` skill 引導
...
| Test | 測試覆蓋率、測試品質 | 所有任務 | `tdd-guide` 覆蓋率分析 |
| Security | 安全性（OWASP Top 10） | 涉及 API/Auth/資料處理 | gstack `/cso` |
...
- 使用 gstack `/ship` 執行：跑測試 → review diff → 更新 CHANGELOG → commit → push → 建 PR
...
7. **若 Skill 未安裝，先依照「前置工具安裝」區塊的指令安裝後再繼續**
